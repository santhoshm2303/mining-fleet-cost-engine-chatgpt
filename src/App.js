          ...
          {results.length===0?<p style={{color:P.txD}}>No results data.</p>:(function(){
            ...
            <div style={cardS}><div style={{padding:"16px 16px 4px",fontWeight:700,color:P.pri,fontSize:13}}>Truck & Charger Fleet by Period</div>
              <ResponsiveContainer width="100%" height={300}><ComposedChart data={truckChargerChartData} margin={{top:10,right:20,left:10,bottom:40}}>
                ...
                <Legend wrapperStyle={{fontSize:11}}/>{fleetTypeNames.map(function(n,idx){var c=mClr[idx % mClr.length]; return <Bar key={"bar-"+n} yAxisId="left" dataKey={"truck__"+n} fill={c} name={n+" Trucks"}/>;})}{fleetTypeNames.map(function(n,idx){var c=mClr[idx % mClr.length]; return <Line key={"line-"+n} yAxisId="right" type="monotone" dataKey={"chg__"+n} stroke={c} strokeWidth={2} name={n+" Chargers"} dot={{r:3}} connectNulls/>;})}
