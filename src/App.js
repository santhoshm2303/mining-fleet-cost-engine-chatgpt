...
export default function App(){
  ...
  const rollupResultRows=(rows)=>{const m={}; rows.forEach(function(r){var k=chartRollup==="year"?getYearLabel(r.periodLabel):r.periodLabel; if(!m[k])m[k]={periodLabel:k,Ore:0,Waste:0,RampBuild:0,oreWt:0,Fe:0,Si:0,Al:0,P:0,TruckCapex:0,DiggerCapex:0,TruckOpex:0,DiggerOpex:0,RehandleOpex:0,ChargerCapex:0,BatteryCapex:0,TruckCPTNum:0,DiggerCPTNum:0,totalMined:0,Trucks:0,Diggers:0,Chargers:0,Diesel:0,Maint:0,Parts:0,GET:0,Operator:0,Other:0,res:null,truckTypeMap:{},chargerTypeMap:{},truckOpexTypeMap:{},__truckBySub:{},__chargerBySub:{},__truckOpexBySub:{}}; var t=m[k]; var ore=(r.pd?.oreMined||0)*scn.unitMul; var waste=(r.pd?.wasteMined||0)*scn.unitMul; var ramp=(r.pd?.totalRampMined||0)*scn.unitMul; var mined=(r.pd?.totalMined||0)*scn.unitMul; t.Ore+=ore; t.Waste+=waste; t.RampBuild+=ramp; t.totalMined+=mined; t.oreWt+=ore; t.Fe+=ore*((r.pd?.oreFePct)||0); t.Si+=ore*((r.pd?.oreSiPct)||0); t.Al+=ore*((r.pd?.oreAlPct)||0); t.P+=ore*((r.pd?.orePPct)||0); t.TruckCapex+=r.res?.trkCapex||0; t.DiggerCapex+=r.res?.digCapex||0; t.TruckOpex+=r.res?.totTrkExc||0; t.DiggerOpex+=r.res?.digOpxTotal||0; t.RehandleOpex+=r.res?.digRehandle||0; t.ChargerCapex+=r.res?.chgCapex||0; t.BatteryCapex+=r.res?.totReplBatCost||0; t.TruckCPTNum+=(r.res?.trkPerT||0)*mined; t.DiggerCPTNum+=(r.res?.digOpxPerT||0)*mined; t.Diggers=Math.max(t.Diggers,r.res?.digFleet||0); t.Diesel+=r.res?.digOpxDiesel||0; t.Maint+=r.res?.digOpxMaint||0; t.Parts+=r.res?.digOpxParts||0; t.GET+=r.res?.digOpxGET||0; t.Operator+=r.res?.digOpxOperator||0; t.Other+=(r.res?.digOpxOil||0)+(r.res?.digOpxCable||0)+(r.res?.digOpxTracks||0)+(r.res?.digOpxTires||0)+(r.res?.digOpxFMS||0)+(r.res?.digOpxBattery||0)+(r.res?.digOpxMaterials||0); var truckType=r.truckName||r.fleetName||"Truck"; var truckCount=Number(r.res?.trkReqR||0); var chargerCount=Number(r.res?.chgStaRnd||0); var truckOpex=Number(r.res?.totTrkExc||0); if(chartRollup==="year"){ var sp=r.periodLabel||k; if(!t.__truckBySub[sp]) t.__truckBySub[sp]={}; if(!t.__chargerBySub[sp]) t.__chargerBySub[sp]={}; if(!t.__truckOpexBySub[sp]) t.__truckOpexBySub[sp]={}; t.__truckBySub[sp][truckType]=(t.__truckBySub[sp][truckType]||0)+truckCount; t.__chargerBySub[sp][truckType]=(t.__chargerBySub[sp][truckType]||0)+chargerCount; t.__truckOpexBySub[sp][truckType]=(t.__truckOpexBySub[sp][truckType]||0)+truckOpex; } else { t.truckTypeMap[truckType]=(t.truckTypeMap[truckType]||0)+truckCount; t.chargerTypeMap[truckType]=(t.chargerTypeMap[truckType]||0)+chargerCount; t.truckOpexTypeMap[truckType]=(t.truckOpexTypeMap[truckType]||0)+truckOpex; t.Trucks+=truckCount; t.Chargers+=chargerCount; }}); return Object.values(m).map(function(t){ if(chartRollup==="year"){ var tmap={}; Object.keys(t.__truckBySub||{}).forEach(function(sp){var mm=t.__truckBySub[sp]||{}; Object.keys(mm).forEach(function(n){tmap[n]=Math.max(tmap[n]||0, mm[n]||0);});}); var cmap={}; Object.keys(t.__chargerBySub||{}).forEach(function(sp){var mm=t.__chargerBySub[sp]||{}; Object.keys(mm).forEach(function(n){cmap[n]=Math.max(cmap[n]||0, mm[n]||0);});}); var omap={}; Object.keys(t.__truckOpexBySub||{}).forEach(function(sp){var mm=t.__truckOpexBySub[sp]||{}; Object.keys(mm).forEach(function(n){omap[n]=(omap[n]||0)+(Number(mm[n])||0);});}); t.truckTypeMap=tmap; t.chargerTypeMap=cmap; t.truckOpexTypeMap=omap; t.Trucks=Object.values(tmap).reduce(function(a,b){return a+(Number(b)||0);},0); t.Chargers=Object.values(cmap).reduce(function(a,b){return a+(Number(b)||0);},0);} var oreC=t.oreWt||1; var minedC=t.totalMined||1; delete t.__truckBySub; delete t.__chargerBySub; delete t.__truckOpexBySub; return Object.assign(t,{Fe:t.Fe/oreC,Si:t.Si/oreC,Al:t.Al/oreC,P:t.P/oreC,TruckCPT:t.totalMined>0?t.TruckCPTNum/minedC:0,DiggerCPT:t.totalMined>0?t.DiggerCPTNum/minedC:0});});};
  ...
  return(
    <div style={{minHeight:"100vh",background:P.bg,color:P.tx,fontFamily:ff}}>
      ...
      <div style={{padding:"20px 32px 60px",width:"100%",boxSizing:"border-box"}}>
        ...
        {/* ══ SETTINGS ══ */}
        {page==="other"&&(<div style={{width:"100%"}}><ST icon="⚙️">General Assumptions</ST><div style={{...cardS,overflowX:"auto",width:"100%"}}>
          <table style={{borderCollapse:"collapse",fontFamily:ff,fontSize:12,width:"100%"}}>
        ...
        {page==="charts_results"&&(<div>
          ...
          {results.length===0?<p style={{color:P.txD}}>No results data.</p>:(function(){
            ...
            var fleetTypeNames=[]; pData.forEach(function(r){Object.keys(r.truckTypeMap||{}).forEach(function(n){if(fleetTypeNames.indexOf(n)===-1)fleetTypeNames.push(n);}); Object.keys(r.chargerTypeMap||{}).forEach(function(n){if(fleetTypeNames.indexOf(n)===-1)fleetTypeNames.push(n);});});
            var truckChargerChartData=pData.map(function(r){var o={period:r.periodLabel}; fleetTypeNames.forEach(function(n){o["truck__"+n]=Number((r.truckTypeMap||{})[n]||0); o["chg__"+n]=Number((r.chargerTypeMap||{})[n]||0);}); return o;})
            ...
            <div style={cardS}><div style={{padding:"16px 16px 4px",fontWeight:700,color:P.pri,fontSize:13}}>Truck & Charger Fleet by Period</div>
              <ResponsiveContainer width="100%" height={300}><ComposedChart data={truckChargerChartData} margin={{top:10,right:20,left:10,bottom:40}}>
                <CartesianGrid strokeDasharray="3 3" stroke={P.bd}/><XAxis dataKey="period" fontSize={10} angle={-20} textAnchor="end"/><YAxis yAxisId="left" fontSize={10} allowDecimals={false}/><YAxis yAxisId="right" orientation="right" fontSize={10} allowDecimals={false}/><Tooltip formatter={function(v){return fmtInt(v)}}/>
                <Legend wrapperStyle={{fontSize:11}}/>{fleetTypeNames.map(function(n,idx){var c=mClr[idx % mClr.length]; return <Bar key={"bar-"+n} yAxisId="left" dataKey={"truck__"+n} stackId="trucks" fill={c} name={n+" Trucks"}/>;})}{fleetTypeNames.map(function(n,idx){var c=mClr[idx % mClr.length]; return <Line key={"line-"+n} yAxisId="right" type="monotone" dataKey={"chg__"+n} stroke={c} strokeWidth={2} name={n+" Chargers"} dot={{r:3}} connectNulls/>;})}
            ...
            {fleetTypeNames.map(function(n,idx){var c=mClr[idx % mClr.length]; return <div key={"truck-opx-"+n} style={cardS}><div style={{padding:"16px 16px 4px",fontWeight:700,color:P.pri,fontSize:13}}>{n+" Truck Opex by Period"}</div>
              <ResponsiveContainer width="100%" height={260}><BarChart data={pData.map(function(r){return{period:r.periodLabel,TruckOpex:Number((r.truckOpexTypeMap||{})[n]||0)}})} margin={{top:10,right:20,left:10,bottom:40}}>
                <CartesianGrid strokeDasharray="3 3" stroke={P.bd}/><XAxis dataKey="period" fontSize={10} angle={-20} textAnchor="end"/><YAxis fontSize={10} tickFormatter={function(v){return "$"+(v/1e6).toFixed(1)+"M"}}/><Tooltip formatter={function(v){return fmtCur(v)}}/>
                <Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="TruckOpex" fill={c} name={n+" Truck Opex"}/>
              </BarChart></ResponsiveContainer>
            </div>;})}
          </div>)})()}
        </div>)}
